...
# Step 5: Define gene sets for tumor_core and tumor_infiltrating
tumor_core_genes <- log2_fc_df_sorted %>%
  filter(tumor_region == "tumor_core") %>%
  pull(sample_id)

tumor_infiltrating_genes <- log2_fc_df_sorted %>%
  filter(tumor_region == "tumor_infiltrating") %>%
  pull(sample_id)

tumor_core_genes <- gsub("^Stanford-", "", tumor_core_genes)
tumor_infiltrating_genes <- gsub("^Stanford-", "", tumor_infiltrating_genes)


# Create a list of gene sets
gene_sets <- list(
  "Tumor_Core" = tumor_core_genes,
  "Tumor_Infiltrating" = tumor_infiltrating_genes
)

# Automatically calculate minSize and maxSize based on the gene sets
minSize <- min(sapply(gene_sets, length)) 
maxSize <- max(sapply(gene_sets, length))  

# Step 6: Perform GSEA using the fgseaMultilevel function
fgsea_results <- fgseaMultilevel(pathways = gene_sets, 
                                 stats = ranked_list, 
                                 minSize = minSize,   #
                                 maxSize = maxSize)   
...
# Step 8: Plot enrichment score for tumor_core
tumor_core_plot <- plotEnrichment(gene_sets[["Tumor_Core"]], ranked_list) +
...
tumor_infiltrating_plot <- plotEnrichment(gene_sets[["Tumor_Infiltrating"]], ranked_list) +
